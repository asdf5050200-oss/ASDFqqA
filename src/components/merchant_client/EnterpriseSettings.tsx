...
export default function EnterpriseSettings({ 
  ...
  // 🌍 语言 (Lingue) Settings States
  const [defaultLanguage, setDefaultLanguage] = useState<string>('it');
  const [languages, setLanguages] = useState([
    { code: 'it', name: 'Italiano (意大利语)', status: 'Default', count: 1840 },
    { code: 'en', name: 'English (英语)', status: 'Published', count: 1840 },
    { code: 'fr', name: 'Français (法语)', status: 'Published', count: 1420 },
    { code: 'zh', name: '中文 (简体)', status: 'Draft', count: 205 }
  ]);
  ...
  const settingsCategories = [
    ...
    { id: 'lingue', name: '🌍 语言设置', desc: '多语言翻译与本地化', icon: Globe },
...
class EventServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        Event::listen(ProductQuantityUpdatedEvent::class, function ($event) {
          ...
          {/* 🌍 语言设置 (Lingue/Languages) */}
          {activeTab === 'lingue' && (
            <div className="space-y-5 animate-fadeIn text-xs">
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 space-y-4 text-left">
                <span className="text-[10px] uppercase font-black text-slate-500 tracking-wider block">
                  🌍 商店默认与发布的多语言清单 (Store Language Configuration)
                </span>

                <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-white p-4 rounded-xl border border-slate-150">
                  <div className="space-y-1">
                    <span className="font-extrabold text-slate-800 text-[12px] block">选择商店默认主控语言 (Primary Language)</span>
                    <span className="text-[9.5px] text-slate-400 block leading-relaxed">
                      这是面向一般访客渲染的首要显示语言，直接绑定 SEO 指引头。
                    </span>
                  </div>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setDefaultLanguage(newCode);
                      setLanguages(p => p.map(l => l.code === newCode ? { ...l, status: 'Default' } : (l.status === 'Default' ? { ...l, status: 'Published' } : l)));
                      addLog('Language Engine', '切换出境站点主控语言', `商店主控默认显示语系正式确定为 [${newCode.toUpperCase()}]`, 'success');
                    }}
                    className="bg-slate-50 border border-slate-250 rounded-lg p-2 font-bold text-xs focus:outline-none"
                  >
                    {languages.map(l => (
                ...
                <div className="overflow-hidden border border-slate-200 bg-white rounded-xl">
                  <table className="w-full text-left text-[11px]">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-150 text-[10px] font-bold text-slate-450 h-8">
                        <th className="p-3 pl-4">语言代键</th>
                        <th className="p-3">语系名称</th>
                        <th className="p-3 text-right">已汉化翻译词条 (Items translated)</th>
                        <th className="p-3 text-center">当前发布状态</th>
                        <th className="p-3 text-center">翻译控制与AI修整 (Actions)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 font-medium text-slate-600">
                      {languages.map(lang => (
                        <tr key={lang.code} className="h-9 hover:bg-slate-50">
                          <td className="p-3 pl-4 font-mono font-bold text-slate-800 uppercase">{lang.code}</td>
                          <td className="p-3 font-semibold">{lang.name}</td>
                          <td className="p-3 text-right font-mono font-bold text-slate-800">{lang.count} / 1840 条</td>
                          <td className="p-3 text-center">
                            <span className={`text-[8.5px] font-bold px-1.5 py-0.5 rounded border uppercase ${
                              lang.status === 'Default' 
                                ? 'bg-[#07C2E3]/10 border-[#07C2E3]/20 text-[#07C2E3]' 
                            ...
                            <div className="flex items-center justify-center gap-1.5">
                              {lang.status !== 'Default' && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    setLanguages(p => p.map(l => l.code === lang.code ? { ...l, status: 'Published' } : l));
                                    addLog('Language Engine', '上线的翻译包部署', `语言包 [${lang.name}] 成功正式向欧区发布。`, 'success');
                                  }}
                                  disabled={lang.status === 'Published'}
                                  className="text-[9px] bg-slate-900 text-white font-bold px-2 py-0.5 rounded disabled:opacity-40 hover:bg-black cursor-pointer leading-none"
                                >
                                  正式发布
                                </button>
                              )}
                              {lang.status !== 'Default' && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    setLanguages(p => p.filter(l => l.code !== lang.code));
                                    addLog('Language Engine', '吊销多语种支持网关', `解除了网站对语系 [${lang.name}] 的国际化翻译支持`, 'warning');
                                  }}
                ...
                {/* Form to Add language and AI Trigger */}
                <div className="pt-2 border-t border-slate-150 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-3 shadow-xs">
                    <span className="font-extrabold text-slate-800 text-[11px] block">➕ 登记新国际化小语种支持</span>
                    <div className="flex gap-2">
                      <select
                        value={newLangCode}
                        onChange={(e) => setNewLangCode(e.target.value)}
                        className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs focus:outline-none"
                      >
                        <option value="">-- 选择目标语系 --</option>
                        <option value="de">Deutsch (德语 - 欧区大国)</option>
                        <option value="es">Español (西班牙语 - 西语大国)</option>
                        <option value="ja">日本語 (日语 - 日韩海运)</option>
                        <option value="pt">Português (葡萄牙语)</option>
                        <option value="nl">Nederlands (荷兰语 - 鹿特丹直发)</option>
                      </select>
                      <button
                        type="button"
                        onClick={() => {
                          ...
                          const presets: any = {
                            de: { name: 'Deutsch (德语)', count: 0 },
                            es: { name: 'Español (西班牙语)', count: 0 },
                            ja: { name: '日本語 (日语)', count: 0 },
                            pt: { name: 'Português (葡萄牙语)', count: 0 },
                            nl: { name: 'Nederlands (荷兰语)', count: 0 }
                          };
                          const p = presets[newLangCode];
                          if (p) {
                            setLanguages(prev => [...prev, { code: newLangCode, name: p.name, status: 'Draft', count: p.count }]);
                            addLog('Language Engine', '登记小语种待译包', `成功登记新出海意向语系: ${p.name}，已自动启动本地化词目排班骨架。`, 'info');
                            setNewLangCode('');
                          }
                        }}
                        disabled={!newLangCode}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-1 px-3.5 rounded-lg text-xs cursor-pointer select-none disabled:opacity-40"
                      >
                        添加
                      </button>
                    </div>
                  ...
                  {/* Gemini AI Auto Localization Panel */}
                  <div className="p-4 bg-[#e6fafc] border border-[#07C2E3]/30 rounded-xl space-y-2.5 shadow-xs">
                    <div className="flex items-center gap-1.5">
                      <Sparkles className="w-4 h-4 text-[#07C2E3] animate-pulse" />
                      <span className="font-extrabold text-[#07C2E3] text-[11.5px]">Gemini 智能一键对账与多语种精修</span>
                    </div>
                    <p className="text-[10px] text-slate-500 font-sans leading-relaxed">
                      智能协同底盘可直接读取您在 <b>Botble CMS / Shopify</b> 所有的产品详情与静态页面骨架，并秒级转换为精准的地道本土表达。
                    </p>
                    <button
                      type="button"
                      ...
                      onClick={() => {
                        ...
                        addLog('Gemini Translators', '启动多语种全量对译修正', '全租户商城整机数据向英、法、德等各主要市场拉取并进行本土化AI精修翻译...', 'info');
                        setTimeout(() => {
                          setIsTranslating(false);
                          setLanguages(p => p.map(l => l.status === 'Draft' ? { ...l, count: 1840, status: 'Published' } : { ...l, count: 1840 }));
                          addLog('Gemini Translators', '全量商城本土语言包渲染清空', '✓ 编译级整机多语言词汇对账成功！已全部对准欧区税收语言政策，1840 个商品变体、付款表单信息完全汉化。', 'success');
                        }, 2000);
                      }}
                      className="w-full flex items-center justify-center gap-1 bg-slate-900 hover:bg-black text-[#07C2E3] hover:text-white font-bold py-1.5 px-3 rounded-lg text-xs cursor-pointer shadow-sm transition-all"
                    >
                      {isTranslating ? (
                        <>
                          <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                          <span>正在执行多语种骨架AI对译对账...</span>
                        </>
                      ) : (
                        <>
                          <Sparkles className="w-3.5 h-3.5 text-[#07C2E3]" />
                          <span>一键开启 Gemini 全站本土化 AI 自动翻译</span>
                        </>
                      )}
                    </button>
                  </div>
